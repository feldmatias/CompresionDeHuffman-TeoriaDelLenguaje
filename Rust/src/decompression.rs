...
fn get_char(bytes: &Vec<u8>, huff_tree: &HuffmanCompression, byte_to_read: &mut usize, read_bits: u8,
    ...
    while !was_letter_decoded {
        ...
        aux_byte = (bytes[*byte_to_read] << _read_bits) >> (_read_bits + BITS_PER_BYTE - (_read_bits + 1));//El ultimo shift da siempre 7
        tree_code.push(std::char::from_u32(aux_byte as u32 + b'0' as u32).expect("Invalid number for tree code"));
        _read_bits += 1;
...
fn execute_decompression(file_name: &String, decompressed_file_text: &mut String) {
    ...
    while byte_to_read < compressed_file.len() {
        ...
        if byte_to_read >= compressed_file.len() {
            byte_to_read = compressed_file.len() - 1;
        }
